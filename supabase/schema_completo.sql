...
-- Esquema completo para una base nueva.
-- Pegalo en el SQL Editor de Supabase y ejecutalo de una vez.
--
-- Si ya venías con el esquema viejo (proyectos/facturas), no uses este archivo:
-- corré en orden las migraciones de supabase/migrations/.
-- ============================================================

-- ------------------------------------------------------------
-- 0. Catálogo: zonas, rubros y cobradores
-- ------------------------------------------------------------
-- clients.zona, clients.cobrador y loans.cobrador los referencian por NOMBRE
-- con ON UPDATE CASCADE: renombrar acá propaga solo a todos los registros.
...
create table if not exists public.clients (
    ...
    cobrador    varchar(150) references public.cobradores (nombre)
                on update cascade on delete set null,
    rubro       varchar(100) references public.rubros (nombre)
                on update cascade on delete set null,
    -- Heredadas del esquema anterior; ya no se piden en el alta
    company     varchar(255),
...
create table if not exists public.payments (
    ...
    -- Sin FK a propósito: es el registro histórico de quién cobró, tiene que
    -- sobrevivir a la baja del cobrador.
...
-- ------------------------------------------------------------
-- 6. El plan de cuotas se genera solo al otorgar el préstamo
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 7. Cada cobro actualiza la cuota y cierra el préstamo si corresponde
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 8. Vista de cobranza: una fila por cuota con zona y cobrador
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 9. RLS + policies de desarrollo
-- ------------------------------------------------------------
-- ATENCIÓN: estas policies dan acceso total a cualquiera que tenga la clave
-- anon, que viaja en el frontend. Para producción usá 003_rls_per_user.sql.
...
create policy "dev_full_access_zonas"        on public.zonas        for all to anon, authenticated using (true) with check (true);
create policy "dev_full_access_rubros"       on public.rubros       for all to anon, authenticated using (true) with check (true);
create policy "dev_full_access_cobradores"   on public.cobradores   for all to anon, authenticated using (true) with check (true);
create policy "dev_full_access_clients"      on public.clients      for all to anon, authenticated using (true) with check (true);
create policy "dev_full_access_interactions" on public.interactions for all to anon, authenticated using (true) with check (true);
create policy "dev_full_access_loans"        on public.loans        for all to anon, authenticated using (true) with check (true);
create policy "dev_full_access_installments" on public.installments for all to anon, authenticated using (true) with check (true);
create policy "dev_full_access_payments"     on public.payments     for all to anon, authenticated using (true) with check (true);


-- ============ RECIBOS Y RENDICIONES (= migrations/008) ============
-- ===================================================
